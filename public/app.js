    ...
    if (instance.drawsHealth) {
        ...
        if (health < 0.99 || shield < 0.99) {
            ...
            if (shield || settings.graphical.seperatedHealthbars) {
                if (!settings.graphical.seperatedHealthbars) ctx.globalAlpha = (1 + shield) * 0.4 * (alpha ** 1.0) * fade;
                drawBar(x - size, x - size + 2 * size * shield, yy, barWidth * 2, settings.graphical.coloredHealthbars ? gameDraw.mixColors(col, color.guiblack, 0.25) : color.teal);
                ctx.globalAlpha = 1;
            }
        }
    }
...
function drawMinimapAndDebug(spacing, alcoveSize) {
    ...
    // Text
    if (global.showDebug) {
        drawText("Swirras 0.4", x + len, y - 50 - 5 * 14 - 2, 15, "#B6E57C", "right");
        ...
        drawText("Swirras 0.4", x + len, y - 50 - 2 * 14 - 2, 15, "#B6E57C", "right");
        drawText((100 * gui.fps).toFixed(2) + "% : " + global.metrics.rendertime + " FPS", x + len, y - 50 - 1 * 14, 10, global.metrics.rendertime > 10 ? color.guiwhite : color.orange, "right");
...
let getDeath = () => {
    ...
    if (global.finalKillers.length) {
        ...
        txt += "🤷 Succumbed from an unknown object";
    }
    return txt;
};
const gameDrawDead = () => {
