...
class io_uncontrolledWhirlwind extends IO {
  constructor(body, opts = {}) {
    super(body);
    this.body.angle = 0
    this.body.dist = opts.initialDist || 110
  ...
  think(input) {
    this.body.angle += (this.body.skill.spd * 2 + this.body.aiSettings.SPEED) * Math.PI / 180;
    /*if(input.fire){
      if(this.body.dist <= 160) this.body.dist += this.radiusScalingSpeed
    }
    else if(input.alt){
      if(this.body.dist >= 100) this.body.dist -= this.radiusScalingSpeed
    }*/
  }
}
...
let ioTypes = {
	...
	uncontrolledWhirlwind: io_uncontrolledWhirlwind,
