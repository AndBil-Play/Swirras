...
function advancedcollide(my, n, doDamage, doInelastic, nIsFirmCollide = false) {
    ...
    if (doDamage) {
        ...
        /********** DO DAMAGE *********/
        let bail = false;
        if (n.type === 'genericTank' && my.settings.necroTypes.includes(n.shape)) {
            bail = my.necro(n);
        } else if (my.type === 'genericTank' && n.settings.necroTypes.includes(my.shape)) {
            bail = n.necro(my);
        }
        if (!bail) {
...
function reflectCollide(wall, bounce) {
    ...
    let bounceBy = bounce.type === 'tank' ? 1.0 : bounce.type === 'miniboss' ? 2.5 : bounce.type === 'drone' ? 2.5 : bounce.type === 'swarm' ? 2.5 : bounce.type === 'trap' ? 2.5 : 0.1
    ...
    if (intersected) {
        if (bounce.type !== 'tank' && bounce.type !== 'miniboss' && bounce.type !== 'food' && bounce.type !== 'drone' && bounce.type !== 'swarm' && bounce.type !== 'trap' && bounce.type !== 'bulletsmasher' && bounce.type !== 'haze' && bounce.type !== 'chain' && bounce.type !== 'explosionchain') {
            bounce.kill(wall);
        } else {
